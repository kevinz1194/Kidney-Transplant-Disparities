...
<!-- All adult donor candidates/Table 1 (Line 55) -->
<!-- Average EPTS by age group and race, all donors/Figure 1A (Line 82) -->
<!-- Average EPTS by age group and race, all donors, table (Line 129)  -->
<!-- Proportion of candidates with EPTS < 20 by age group and race, all donors/Figure 1B (Line 159)  -->
<!-- Logistic regression for EPTS, includes living donors (Line 214) -->
<!-- Survival plots for competing risks/Figure 2 (Line 243) -->
<!-- Top 20% KDPI for top 20% EPTS only (Line 346) -->
<!-- Adjusted results for age (Line 378) -->
...
Note: Legends and titles must be manually added.
```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center', fig.width=10, fig.height=10}
cuminc_age_adjusted <- cuminc(Surv(wait_time / 365.25, status_top20) ~ race + age_group, data = df_cand_kipa_cmp_all_race)
...
plot_30_39 <- ggcuminc(cuminc_age_adjusted, 
  ...
  xlim(0, 5) + ylim(0, 0.4) +
  theme_bw() + 
  scale_color_manual(values = c(rep(alpha('white', 0), 7), rep(alpha('white', 0), 7), 
...
cowplot::plot_grid(plot_30_39, plot_40_49, plot_50_59, align = 'h', ncol = 3)
```
