...
<!-- All adult deceased donor candidates, no 2015 (Line 49) -->
<!-- Average EPTS by age group and race, no 2015 (Line 74) -->
<!-- Average EPTS by age group and race, no 2015 (Line 104)  -->
<!-- Proportion of candidates with EPTS < 20 by age group and race, no 2015 (Line 178)  -->
<!-- Proportion of candidates with EPTS < 20 by age group and race, no 2015 (Line 229) -->
<!-- -->
<!-- Average KDPI by age group and race, no 2015 (Line 266) -->
<!-- Average KDPI by age group and race, no 2015 (Line 309)  -->
<!-- Proportion of candidates with KDPI < 20 by age group and race, no 2015 (Line 338) -->
<!-- Proportion of candidates with KDPI < 20 by age group and race, no 2015 (Line 372) -->
<!-- -->
<!-- Logistic regression for EPTS, no 2015 (Line 414) -->
<!-- Logistic regression for KDPI, no 2015 (Line 427)  -->
<!-- -->
<!-- Survival plots for competing risks, no 2015 (Line 445) -->
<!-- Overall results for top 20% EPTS only, no 2015 (Line 510) -->
<!-- Adjusted results for age, no 2015 (Line 536) -->
<!-- Cumulative incidence of top deceased donor KDPI transplant, with various competing risks (incl. living donor) (Line 665) -->
<!-- Cumulative incidence of top deceased donor KDPI transplant, with various competing risks (incl. living donor) (Line 713) --> 
...
### All adult deceased donor candidates, no 2015
...
tbl_summary(df_cand_kipa_table1, by = race,
  ...
  bold_labels() %>%
  add_overall() %>%
  modify_caption("**All Adult Deceased Donor Candidates in Top 20% EPTS**") 
...
### Average EPTS by age group and race, no 2015
...
### Proportion of candidates with EPTS < 20 by age group and race, no 2015 (Figure 2)
```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center'}
...
### Proportion of candidates with EPTS < 20 by age group and race, no 2015 (Table 3)
```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center'}
...
### Average KDPI by age group and race, no 2015 (plot)
```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center'}
df_cand_kipa_kdpi1 <- 
  df_cand_kipa %>%
     group_by(race, age_group) %>%
...
### Average KDPI by age group and race, no 2015 (table)
```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center'}
...
### Proportion of candidates with KDPI < 20 by age group and race, no 2015 (plot)
```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center'}
...
### Proportion of candidates with KDPI < 20 by age group and race, no 2015 (table)
```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center'}
...
### Logistic regression for KDPI, no 2015
```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center'}
...
### Overall results for top 20% EPTS only, no 2015
```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center', fig.width=10, fig.height=10}
...
### Cumulative incidence of top deceased donor KDPI transplant, with various competing risks (incl. living donor)
We filter to those who are White, Black or AA, and Hispanic due to small counts in the other race categories. \newline

These levels correspond to the possible outcomes: \newline

* 0: Censored
* **1: Transplant with top 20% kidney from deceased donor** (main outcome)
* 2: Transplant with non-top 20% kidney from deceased donor
* 3: Transplant from living donor
* 4: Death or delisting


```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center', fig.width=9, fig.height=9, results='hide'}
rm(list=ls())
load('C:/Users/kevinz94/Desktop/EPTS_Score/candidates_livingdonor.RData')

df_cand_kipa$race <- droplevels(df_cand_kipa$race)
df_cand_kipa <- df_cand_kipa %>%
  filter(race == 'White' | race  == 'Black_AA' | race  == 'Hispanic')
df_cand_kipa$race <- factor(df_cand_kipa$race, levels=c('White', 'Black_AA', 'Hispanic'))


df_cand_kipa <- df_cand_kipa %>%
  mutate(status_top20_deceased = case_when(
    
    top20_kdpi == '1' & living_donor == '0' ~ 1,
    not_top20_kdpi == '1' & living_donor == '0' ~ 2,
    living_donor == '1' & transplant == '1' ~ 3,
    death == '1' | is.na(removal_code) == T ~ 4,
    TRUE ~ 0)) %>%
  
  mutate(status_top20_deceased = factor(status_top20_deceased, levels=c(0,1,2,3,4)))


top20_deceased_cuminc <- cuminc(Surv(time / 365.25, status_top20_deceased) ~ race, data = df_cand_kipa) 


ggcuminc(top20_deceased_cuminc, 
         outcome = '1', size = 1.2) + 
  labs(x = 'Time (years)', y = 'Cumulative Incidence') +
  ggtitle('Top 20% Deceased Donor KDPI Transplant with Death, Living Donor, Other Deceased Donor Competing Risks') +
  scale_color_manual(values = c('firebrick', '#009E73', '#56B4E9')) +
  xlim(c(0,5)) + 
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
### Cumulative incidence of top deceased donor KDPI transplant in top EPTS, with various competing risks (incl. living donor)

Only those in the top 20% EPTS percentile are considered. \newline

```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center', fig.width=9, fig.height=9, results='hide'}
df_cand_kipa_top20 <- df_cand_kipa %>%
  filter(top_percentile_epts == '1')



df_cand_kipa_top20 <- df_cand_kipa_top20 %>%
  mutate(status_top20_deceased = case_when(
...
top20_deceased_cuminc <- cuminc(Surv(time / 365.25, status_top20_deceased) ~ race, data = df_cand_kipa_top20) 


ggcuminc(top20_deceased_cuminc, 
         outcome = '1', size = 1.2) + 
  labs(x = 'Time (years)', y = 'Cumulative Incidence') +
  ggtitle('Top 20% Deceased Donor KDPI Transplant in Top EPTS with Death, Living Donor, Other Deceased Donor Competing Risks') +
  scale_color_manual(values = c('firebrick', '#009E73', '#56B4E9')) +
  xlim(c(0,5)) + 
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
