...
<!-- All adult deceased donor candidates, no 2015 (Line 43) -->
<!-- Average EPTS by age group and race, deceased donors, no 2015 (Line 70) -->
<!-- Average EPTS by age group and race, deceased donors, no 2015, table (Line 115)  -->
<!-- Proportion of candidates with EPTS < 20 by age group and race, deceased donors, no 2015 (Line 145)  -->
<!-- Proportion of candidates with EPTS < 20 by age group and race, deceased donors, no 2015 (Line 197) -->
<!-- -->
<!-- Logistic regression for EPTS, includes living donors, no 2015 (Line 234) -->
<!-- Logistic regression for KDPI, includes living donors, no 2015 (Line 248)  -->
<!-- -->
<!-- Survival plots for competing risks, no 2015 (Line 276) -->
<!-- Overall results for top 20% EPTS only, no 2015 (Line 371) -->
<!-- Adjusted results for age, no 2015 (Line 399) -->
...
df_cand_kipa <- subset(df_cand_kipa, year(as.Date(df_cand_kipa$listing_date)) != 2015)
...
### Average EPTS by age group and race, deceased donors, no 2015
...
### Average EPTS by age group and race, deceased donors, no 2015, table
```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center'}
...
### Proportion of candidates with EPTS < 20 by age group and race, deceased donors, no 2015
```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center'}
...
### Proportion of candidates with EPTS < 20 by age group and race, deceased donors, no 2015
```{r, warning=F, echo=F, comment=NA, message=F, fig.align='center'}
df_cand_kipa_table3 <- df_cand_kipa %>% filter(living_donor == '0') %>%
  group_by(race, age_group) %>%
...
df_cand_kipa_cmp <- df_cand_kipa %>%
  ...
  select(PX_ID, time, race, transplant, death, top20_kdpi, removal_code, age_group, top_percentile_epts, living_donor) %>%
  ...
  select(time, race, status_death, status_top20, age_group, top_percentile_epts, living_donor)
