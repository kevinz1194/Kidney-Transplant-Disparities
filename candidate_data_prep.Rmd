...
for(i in 1:max_retransplants) {
  ...
  multiple_registrations %>%
    ...
    mutate(wait_time = case_when(
      is.na(REC_TX_DT) == FALSE & transplant_num != '0' ~ REC_TX_DT- min_list_date, ### Ignore sequential rows
...
## Dataset check on case where transplant can affect dates
```{r, warning=F, comment=NA}
multiple_registrations %>%
  group_by(PERS_ID) %>%
  arrange(PERS_ID, CAN_LISTING_DT) %>% 
